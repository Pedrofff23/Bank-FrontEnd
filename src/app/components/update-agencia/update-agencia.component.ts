...
export class UpdateAgenciaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.utilsService
      .getByTipo(this.utilfiltro)
      .subscribe((response: Utils[]) => {
        this.utils = response;
      });
    this.id = this.route.snapshot.params['id'];
    this.AgenciaService.getOne(this.id).subscribe(
      (data) => {
        this.Agencia = data;
  ...
  public onSubmit() {
    ...
    this.AgenciaService.update(this.id, this.updated).subscribe(
      (data) => {
        this.onSucces();
      },
      (error) => {
